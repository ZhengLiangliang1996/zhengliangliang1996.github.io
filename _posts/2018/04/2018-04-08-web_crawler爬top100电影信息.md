...
28 def write\_to\_mongoDB(item):
29     #content.insert\_one({'index': item\[0\], 'image': item\[1\], 'title': item\[2\], 'actor': item\[3\].strip()\[3:\], 'time':item\[4\].strip()\[5:\],'score':item\[5\] + item\[6\]})
30     content.insert\_one(item)
31 
32 \# Main function
33 def main(offset):
34     url \= 'http://maoyan.com/board/4?offset=' + str(offset)
35     html \= get\_one\_page(url)
36     for item in parse\_one\_page(html):
37         print(item)
38         write\_to\_mongoDB(item)
39 
40 
...
然后对界面进行解析，我们比较常用的是BeautifulSoup来解析，这里我们使用re中的pattern来进行匹配。然后用yield数据结构进行存储。前面建立好数据库，到了后面直接用表的一个Insert\_one方法进行插入，顺爽丝滑
...
![图片1](images/e59bbee789871.png)

数据库

![图片1](images/e59bbee7898711.png)
