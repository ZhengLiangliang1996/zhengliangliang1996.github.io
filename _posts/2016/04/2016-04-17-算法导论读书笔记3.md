...
![](images/302040063916099.png)我们这里所说的堆有两种，分别是**最大堆**和**最小堆**。 **大根堆**:最大堆特性是指除了根以外的每个结点 _i_，有 _A_ \[ `PARENT(i)` \] >= _A_ \[ _i_ \]。即某个结点的值至多和其父结点一样大。这样，堆中的最大元素就存放在根结点中. **小根堆:**最小堆特性是指除了根以外的每个结点 **_i_ ，有 _A_ \[ `PARENT(i)` \] <= _A_ \[ _i_ \]，最小堆的最小元素是在根.** 
...
- Max\_Heapify 过程 运行时间是O(lgn)，是保持最大堆性质的关键
...
1.保持堆的性质  Max\_Heapify ：当 `MAX-HEAPIFY` 被调用时，我们假定以 `LEFT(i)` 和 `RIGHT(i)` 为根的两棵二叉树都是最大堆，但这时 _A_ \[ _i_ \]可能小于其子女，这样就违反了最大堆特性。 `MAX-HEAPIFY` 让 _A_ \[ _i_ \]在最大堆中“下降”，使以_i_ 为根的子树成为最大堆
...
![](images/302039361561864.png)

 

2.建堆 Build\_Max\_Heap(A)
...
![](images/302040416254898.png)

调用了n次的Max\_Heapify()  所以运行时间是O(nlgn)
...
![](images/302040595783983.png)

最终可以得出 `BUILD-MAX-HEAP` 过程运行时间的界为

![](images/302041161566081.png)
...
void MAX\_HEAPIFY(int a\[\], int i, int n)
{
    int l, r, largest;
    l \= 2 \* i;
    r \= (2 \* i + 1);
...
//      a\[i\] = a\[largest\];
//      a\[largest\] = loc;
        MAX\_HEAPIFY(a, largest, n);
    }
}
void BUILD\_MAX\_HEAP(int a\[\], int n)
{
    for (int k \= n / 2; k \>= 1; k\--)
    {
        MAX\_HEAPIFY(a, k, n);
